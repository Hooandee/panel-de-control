...
export interface Pill {
  ...
  desc?: string;
  /** Removed custom definition retained only so active game tokens remain removable. */
  retired?: boolean;
...
// A single-choice pill whose baseline had TWO+ of its flags (e.g. `-dx11 -dx12`):
// neither on nor off, so the rebuild preserves all its flags until the user picks
// one. Distinct from "off" (undefined), which removes the flag.
export const AMBIGUOUS = "__PDC_AMBIGUOUS__";
  ...
  for (const pill of catalog) {
    if (pill.kind === "env" && pill.envName) {
      ...
      if (pill.options) {
        ...
        else if (present.length > 1) out[pill.id] = AMBIGUOUS;
...
export function buildLaunchOptions(
  ...
  for (const pill of catalog) {
    ...
    if (pill.kind === "wrapper" && pill.wrapper) {
      ...
      // Strip a single-choice pill's flags when switching (a value) OR turning it off
      // (undefined) — but NOT when ambiguous (two+ present), where we preserve them
      // until the user resolves it. `active` is false for both off and ambiguous, so
      // key off the AMBIGUOUS marker directly.
      if (selections[pill.id] !== AMBIGUOUS) p = stripPill(p, pill);
    } else {
      p = stripPill(p, pill);
    }
  }
