...
* Panel de Control introduces **Dashboard**, a home screen with cards that take you straight to Power, Display, Sound, and the other sections. If you prefer tabs, that navigation mode is still available. Both modes have improved controller navigation: selections are easier to see, scrolling keeps the selected control in view, and returning to Dashboard takes you back to the card you opened.
...
* Panel de Control introduce **Dashboard**, una schermata iniziale con riquadri per accedere direttamente a Potenza, Schermo, Suono e alle altre sezioni. Se preferisci navigare con le schede, puoi continuare a usare quella modalità. In entrambe abbiamo migliorato la navigazione con il controller: la selezione è più visibile, lo scorrimento mantiene in vista il controllo selezionato e, tornando al Dashboard, ritrovi il riquadro da cui sei partito.
